...
我使用Git已經有4年了，我想與大家分享些實用的技巧。希望能幫助到你。

如果你完全對於Git不清楚，我推薦你先讀過一次[Git Cheat Sheet](http://www.alexkras.com/getting-started-with-git/)。這篇文章撰寫的對象是有3個月以上Git經驗的人。

# 指令清單

1. 善用 log 參數, `git log --online --graph`
2. git log -p filename
3. git log -L 1,1:some-file.txt
4. git log --no-merge master..
5. git show some-branch:some-file.js
6. git pull --rebase
7. git merge --no-ff
8. git commit --amend
9. git reset --hard HEAD and git status -s
10. git revert -n
11. git difftool -d
12. git diff -w
13. git add -p
...
例如，每週五我的主管需要我提交每週的報告。所以每週我會執行: `git log --author="Alex Kras" --after="1 week ago" --oneline`，然後再修飾一下並提交給主管。
...
在與多人共同開發時，你在某個分支開發了很久的時間，當你將父(parent)分支(例如, master)合併進你的分支時。這時你會發現很難看出你開發的分支與master的差異。
...
2. `git reset {{some-commit-hash}}` - 還原到你指定的commit hash。**所有改變都會保留，並且不會加入暫存檔案(unstaged files)**。而你可以在使用`git add .`和`git commit`還原回去。
...
`git add -p`可讓你選擇檔案中哪些異動是你想要加入暫存檔案(staged file)，並且準備commit的。
...
如果你只想看到合併的分支commit，可能需要寫一個script去比對找出合併的分支。(Unfortunately, there is no easy way (that I know of) to only show merged branches. So you might have to just compare the two outputs or write a script to do it for you.)
